        ...
        <div id="js-grid-full-width" class="cbp">
            @foreach(json_decode($gallery->gallery_images) as $image)
                <div class="cbp-item identity logos">
                    <a href="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" class="cbp-caption cbp-lightbox" data-title="Dashboard<br>by Paul Flavius Nechita">
                        <div class="cbp-caption-defaultWrap">
                            <img src="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" data-cbp-src="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" alt="">
                        </div>
                    </a>
                </div>
            @endforeach
    ...
    <section class="gallery_section">
        <div class="container-fluid">
            ...
            <div id="js-grid-mosaic" class="cbp">
                @foreach(json_decode($gallery->gallery_images) as $image)
                    <div class="cbp-item">
                        <a href="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" class="cbp-caption cbp-lightbox">
                            <div class="cbp-caption-defaultWrap">
                                <img src="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" data-cbp-src="{{ URL::to('/') }}/gallery/gallery_images/{{ $gallery->updated_at->format('F_Y') }}/{{ $image }}" alt="">
                            </div>
